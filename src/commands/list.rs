...
pub fn run(file: &Path, attachments_only: bool, base64_only: bool) -> Result<(), MailCrushError> {
    ...
    if attachments_only {
        ...
        let attachments: Vec<_> = summary.parts.iter()
        ...
        if attachments.is_empty() {
            ...
            println!("{:<4} {:<30} {:<15} {:<10}", "#", "Filename", "Type", "Size");
            println!("{}", "─".repeat(60));
            
            for (i, part) in attachments.iter().enumerate() {
                let filename = part.filename.as_deref().unwrap_or("unnamed");
                let short_type = part.content_type.split('/').last().unwrap_or(&part.content_type);
                println!("{:<4} {:<30} {:<15} {} bytes", 
                         i + 1, 
        ...
        let base64_parts: Vec<_> = summary.parts.iter()
