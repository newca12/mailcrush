...
pub fn run(
    ...
    if report.full_reconstruction_verified {
        ...
        let message = mail_parser::MessageParser::default()
            .parse(&raw_content)
            .ok_or_else(|| MailCrushError::ParseError("Failed to parse email for storage".to_string()))?;
        ...
        let parts_original_size: usize = part_infos.iter()
            .map(|p| p.offset_end - p.offset_body)
            .sum();
        let parts_compressed_size: usize = part_infos.iter()
        ...
        println!("   Space savings:         {} bytes ({:.1}%)", savings, savings_pct);
        println!();
